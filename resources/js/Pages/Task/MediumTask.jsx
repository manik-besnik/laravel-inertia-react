...
const MediumTask = ({ account }) => {
    ...
    const { data, setData, errors, post, progress, processing } = useForm({
        ...
        task_type: route().params.type ?? taskType.clap,
        allow_user: '1',
        coin: getTaskCoin(route().params.type),
        task_url: (route().params.type === taskType.like || route().params.type === taskType.follow) ? account?.account_url : null,
    })
    ...
    const coinIcon = () => {
        ...
        if (data.task_type === taskType.clap) {
    ...
    return (
        <>
            <Head title="Create Medium Task | Maketop.me" />

            <AuthenticatedLayout
                header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Medium</h2>}
            >
                ...
                <div className='flex justify-between gap-20 flex-col min-h-screen'>
                    <div>
                        ...
                        <h2 className='app-subtitle'>Create Task for Medium:</h2>

                        <div className="flex items-center justify-start gap-[10px] mb-[30px]">

                            {
                                tabs.map((tab, idx) => (
                                    <Link key={idx} href={tab.isComing ? '' : route('medium.create', { type: tab.typeOfTask })}
                            ...
                            {/* <Link href={route('medium.create', { type: taskType.follow })}
                            ...
                            {/*<Link href={route('medium.create', {type: taskType.clap})}*/}
                            {/*      className={`${data.task_type === taskType.clap ? 'bg-card-and-hover' : ' bg-side-and-button'} px-2 sm:px-3 py-1.5 sm:py-2 rounded sm:rounded-[6px] text-xs sm:text-sm leading-[15px] sm:leading-[20px] font-medium  hover:bg-card-and-hover text-text-primary`}*/}
                            {/*>Clap*/}
                            {/*</Link>*/}

                            {/* <Link href={route('medium.create', { type: taskType.comment })}
                        ...
                        <form onSubmit={handleSubmit} className='max-w-[454px]'>
                            <div className='mb-4 sm:mb-6'>
                                <LabelField content="Link to Medium Profile" />
                                <InputBox disabled={data.task_type === taskType.clap || data.task_type === taskType.follow} value={data.task_url} onChange={e => setData('task_url', e.target.value)}
                            ...
                            {!account && <div
                                ...
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <span className='text-xs leading-[14px] sm:text-sm text-white'>Add Your Medium Profile URL First!</span>
                                </div>
                            </div>}

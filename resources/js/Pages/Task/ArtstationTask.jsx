...
const ArtstationTask = ({account}) => {
    ...
    const {data, setData, errors, post, progress, processing} = useForm({
        account_type: accountType.artstation,
    ...
    return (
        <>
            <Head title="Create Artstation Task | Maketop.me"/>

            <AuthenticatedLayout
                header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Artstation</h2>}
            >
                ...
                <div className='flex justify-between gap-20 flex-col min-h-screen'>
                    <div>
                        ...
                        <h2 className='app-subtitle'>Create Task for Artstation:</h2>
                        ...
                        <div className="flex items-center justify-start gap-[10px] mb-[30px]">

                            {
                                tabs.map((tab, idx) => (
                                    <Link key={idx}
                                          href={tab.isComing ? '' : route('dribbble.create', {type: tab.typeOfTask})}
                        ...
                        <form onSubmit={handleSubmit} className='max-w-[454px]'>
                            <div className='mb-4 sm:mb-6'>
                                <LabelField content="Link to Dribbble Profile"/>
                                <InputBox
                                    value={data.task_url}
                                    disabled={data.task_type === taskType.follow}
                                    onChange={e => setData('task_url', e.target.value)}
                                    placeholder='https://www.example.net/example'
                                    iconPrev={<Copy className='h-4 sm:h-5'/>}
                                />
                                {errors.task_url &&
                                    <ErrorField className="mt-[14px]" content={errors.task_url}/>}
                            </div>
                            ...
                            {!account && <div
                                ...
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <span className='text-xs leading-[14px] sm:text-sm text-white'>Add Your Artstation Profile URL First!</span>
                                </div>
                            </div>}
